...
public class CassandraClient10 extends DB
{
  ...
  private static Logger logger = LoggerFactory.getLogger(CassandraClient10.class);
  ...
  Integer qos_level;
  boolean _debug = true;
  Integer write_qos_level;
  Integer scan_qos_level;
   ...
   */
  public void init(String args1,String args2,String args3) throws DBException
  {
    qos_level=Integer.parseInt(args1);
    write_qos_level=Integer.parseInt(args2);
    scan_qos_level=Integer.parseInt(args3);
    // Initializing log4j
    /*String configFileName= "/root/YCSB/conf/log4j.properties";
    PropertyConfigurator.configureAndWatch(configFileName, 10000);
    org.apache.log4j.Logger.getLogger(Client.class).info("Logging initialized");*/
    logger.debug("In Cassandra Client !!");
    try 
    {
    	throw new RuntimeException("In init of Client10");
    }
    catch (Exception e)
    {
    	logger.debug("qos_level = " + qos_level +" write_qos_level "+write_qos_level+" scan_qos_level "+scan_qos_level+" Stacktrace ", e);
    }
  ...
  {
    logger.debug("In Read wiht table " + table + " key " + key);
	...
	logger.debug("Going for a QoS Read");
	/* We're initializing this here, but this should ideally come from CoreYCSB */
	int QoSLevel=qos_level;
	int WriteQoS=write_qos_level;
	int ScanQoS=scan_qos_level;
	int currentReads= Profiling.incrementAndGetRead();
	logger.debug("Going for for a QoS Write Level {} i",WriteQoS);
	Profiling.writeToFile(1,currentReads);
        List<ColumnOrSuperColumn> results = client.get_slice_QoS(ByteBuffer.wrap(key.getBytes("UTF-8")), parent, predicate, readConsistencyLevel, QoSLevel);
	Profiling.decrementRead();
      ...
      Vector<HashMap<String, ByteIterator>> result)
  {
	...
	//Cassandra Team passing scan qos
	int ScanQoS=scan_qos_level;
        //List<KeySlice> results = client.get_range_slices(parent, predicate, kr, scanConsistencyLevel);
        List<KeySlice> results = client.get_range_slices_QoS(parent, predicate, kr, scanConsistencyLevel,ScanQoS);
    ...
    for (int i = 0; i < OperationRetries; i++)
    {
     ...
     //   System.out.println("Inserting key: " + key);
      ...
      try
      {
        ...
        logger.debug("Write QoS "+write_qos_level);
        client.batch_mutate_QoS(record, writeConsistencyLevel,write_qos_level);
